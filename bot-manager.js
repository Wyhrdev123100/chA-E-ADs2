// ============================================================================
// BOT-MANAGER.JS - KillAura Edition v4.0
// ============================================================================
// Cơ chế KillAura giống Wurst Client:
// - Server-side rotation (bot.look với force=false)
// - Speed = 0 → tự động dùng Minecraft attack cooldown
// - Priority: Angle / Distance / Health
// - FOV filter
// - Pause on containers
// - physicsEnabled: false → không spam packet
// ============================================================================
...
const MathUtils = {
    getRotationTo(from, to) {
        ...
        const pitch = -Math.atan2(dy, dist2D) * (180 / Math.PI);
        return { yaw, pitch };
    },

    angleBetween(yaw1, pitch1, yaw2, pitch2) {
        const toRad = Math.PI / 180;
        const r1y = yaw1 * toRad;
        const r1p = pitch1 * toRad;
        const r2y = yaw2 * toRad;
        const r2p = pitch2 * toRad;
        const v1 = {
            x: -Math.sin(r1y) * Math.cos(r1p),
            y: -Math.sin(r1p),
            z: Math.cos(r1y) * Math.cos(r1p),
        };
        const v2 = {
            x: -Math.sin(r2y) * Math.cos(r2p),
            y: -Math.sin(r2p),
            z: Math.cos(r2y) * Math.cos(r2p),
        };
...
class BotWrapper {
    ...
    connect() {
        ...
        try {
            ...
            this.logger.info(
                "Bot",
                `[${this.options.username}] Kết nối ${this.options.host}:${this.options.port}...`,
            );
        } catch (err) {
            this.status = "error";
            ...
            this.logger.error(
                "Bot",
                `[${this.options.username}] Lỗi: ${err.message}`,
            );
            this.emitUpdate();
        }
    }
    ...
    cleanupBot() {
        ...
        try {
            this.bot.quit("cleanup");
        } catch (e) {}
        this.bot = null;
    }

    // ========================================================================
    // KILLAURA - Tính cooldown
    // ========================================================================

    updateCooldown() {
        if (!this.bot) return;
        const v = parseFloat(
            String(this.bot.version || "1.9").replace(/[^0-9.]/g, ""),
        );

        if (this.killaura.speed === 0) {
            this.killaura.cooldownMs = v <= 1.8 ? 125 : 625;
        } else {
            const cps = Math.min(this.killaura.speed, 12);
            this.killaura.cooldownMs = Math.floor(1000 / cps);
        }
    }
    ...
    findKillauraTarget() {
        ...
        const range = this.killaura.range;
        const fov = this.killaura.fov;
        ...
        for (const entity of Object.values(this.bot.entities)) {
            ...
            if (types.includes("mob") && isMob) ok = true;
            if (!ok) continue;

            // Filters
            if (this.killaura.filterMonsters && isHostile) continue;
            if (this.killaura.filterPlayers && isPlayer) continue;
            if (this.killaura.filterAnimals && isAnimal) continue;
            if (
                this.killaura.filterInvisible &&
                entity.metadata?.find((m) => m.value === true)
            )
                continue;
            if (this.killaura.filterNamed && entity.username) continue;
            ...
            const rot = MathUtils.getRotationTo(eyePos, targetPos);

            // FOV check
            const angle = MathUtils.angleBetween(
                this.killaura.serverYaw,
                this.killaura.serverPitch,
                rot.yaw,
                rot.pitch,
            );
            if (fov < 360 && angle > fov / 2) continue;

            candidates.push({
                entity,
                dist,
                angle,
                health: entity.health || 999,
                rot,
            });
        }
    ...
    // ========================================================================
    // KILLAURA - Start / Stop / Tick
    // ========================================================================

    startKillaura(options = {}) {
        // Cập nhật settings
        if (options.range !== undefined)
            this.killaura.range = Math.min(parseFloat(options.range) || 4.5, 6);
        if (options.speed !== undefined)
            this.killaura.speed = Math.min(parseFloat(options.speed) || 0, 12);
        if (options.priority !== undefined)
            this.killaura.priority = options.priority;
        if (options.fov !== undefined)
            this.killaura.fov = Math.min(parseFloat(options.fov) || 360, 360);
        if (options.pauseOnContainers !== undefined) {
            this.killaura.pauseOnContainers = !!options.pauseOnContainers;
        }
        if (Array.isArray(options.targetTypes)) {
            this.killaura.targetTypes = options.targetTypes;
        }
        ...
        for (const k of [
            ...
            if (options[k] !== undefined) this.killaura[k] = !!options[k];
        }
        ...
        this.currentTask = "⚔️ KillAura";

        this.updateCooldown();

        if (this.intervals.killaura) {
            clearInterval(this.intervals.killaura);
            delete this.intervals.killaura;
        }

        // Interval chạy đúng theo cooldown
        this.intervals.killaura = setInterval(
            () => this.doKillaura(),
            this.killaura.cooldownMs,
        );

        this.logger.info(
            "Bot",
            `[${this.options.username}] ⚔️ KillAura BẬT | range:${this.killaura.range} speed:${this.killaura.speed || "auto"} priority:${this.killaura.priority} fov:${this.killaura.fov}°`,
        );
    }
    ...
    stopKillaura() {
        ...
        this.logger.info(
            "Bot",
            `[${this.options.username}] ⚔️ KillAura TẮT | Clicks: ${this.killaura.totalClicks}`,
        );
    }

    doKillaura() {
        if (!this.killaura.enabled) return;
        if (!this.bot || this.status !== "online" || this.destroyed) return;
        if (!this.bot.entity) return;
        if (this.killaura.pauseOnContainers && this.killaura.containerOpen)
            return;

        const candidate = this.findKillauraTarget();
        ...
        const { entity, dist, rot } = candidate;
        ...
        try {
            // force = false → chỉ gửi packet lên server, không lock camera client
            this.bot.look(rot.yaw, rot.pitch, false).catch(() => {});
        } catch (e) {}

        // Attack
        try {
            this.bot.attack(entity);
            this.killaura.lastAttack = Date.now();
            this.killaura.totalClicks++;

            // Emit mỗi 5 clicks để giảm load
            if (this.killaura.totalClicks % 5 === 0) {
                this.io.to(`bot:${this.id}`).emit("bot:killaura", {
                    botId: this.id,
                    target: this.killaura.currentTarget,
                    totalClicks: this.killaura.totalClicks,
                    cooldownMs: this.killaura.cooldownMs,
                });
            }
        } catch (e) {}
    }
    ...
    async lookAt(x, y, z) {
        if (!this.bot || this.status !== "online")
            throw new Error("Bot chưa online");
    ...
    async attackEntity(entityId) {
        ...
        try {
            await this.bot.lookAt(
                entity.position.offset(0, (entity.height || 1.8) * 0.8, 0),
            );
        } catch (e) {}
    ...
    async attackNearest(type = "hostile") {
        ...
        const entity = this.bot.nearestEntity((e) => {
            ...
            if (MathUtils.distance3D(this.bot.entity.position, e.position) > 5)
                return false;
            if (type === "hostile")
                return e.type === "hostile" || e.kind === "Hostile mobs";
            if (type === "mob") return e.type === "mob";
            if (type === "player")
                return e.type === "player" && e.username !== this.bot.username;
            return e.name === type;
        });
        if (!entity) throw new Error(`Không có ${type} trong tầm`);
        try {
            await this.bot.lookAt(
                entity.position.offset(0, (entity.height || 1.8) * 0.8, 0),
            );
    ...
    setupEvents() {
        ...
        this.bot.on("login", () => {
            ...
            this.logger.info(
                "Bot",
                `[${this.options.username}] ✅ Online! v${this.bot.version} cooldown:${this.killaura.cooldownMs}ms`,
            );
            this.emitUpdate();
        });
        ...
        this.bot.on("spawn", () => {
            ...
            this.timers.spawn = setTimeout(() => {
                if (!this.destroyed && this.status === "online") {
                    ...
                    if (this.killaura.enabled) {
                        this.startKillaura({
                            range: this.killaura.range,
                            speed: this.killaura.speed,
                            priority: this.killaura.priority,
                            fov: this.killaura.fov,
        ...
        this.bot.on("health", () => {
            ...
            this.statsTracker.addToHistory(this.id, "healthHistory", {
                health,
                food,
            });

            if (
                this.autoEatState.enabled &&
                !this.autoEatState.eating &&
                food <= this.autoEatState.startAt
            ) {
        ...
        this.bot.on("respawn", () => {
            ...
            this.killaura.currentTarget = null;
            this.killaura.containerOpen = false;
            this.logger.info("Bot", `[${this.options.username}] 🔄 Respawn`);
        });

        this.bot.on("windowOpen", () => {
            if (this.destroyed) return;
            if (this.killaura.pauseOnContainers) {
                this.killaura.containerOpen = true;
            }
        });
        ...
        this.bot.on("chat", (username, message) => {
            ...
            this.io
                .to(`bot:${this.id}`)
                .emit("bot:chat", { botId: this.id, ...entry });
        ...
        this.bot.on("message", (jsonMsg) => {
            ...
            try {
                ...
                if (text && text.trim()) {
                    ...
                    this.io
                        .to(`bot:${this.id}`)
                        .emit("bot:message", { botId: this.id, ...entry });
                }
            } catch (e) {}
        });
        ...
        this.bot.on("kicked", (reason) => {
            ...
            this.logger.warn(
                "Bot",
                `[${this.options.username}] ⚠️ Kick: ${this.error}`,
            );
            this.emitUpdate();
            this.handleKick(rs);
        });
        ...
        this.bot.on("error", (err) => {
            ...
            if (!ignored.some((e) => err.message?.includes(e))) {
                this.logger.error(
                    "Bot",
                    `[${this.options.username}] ❌ ${err.message}`,
                );
            }
        });
        ...
        this.bot.on("end", (reason) => {
            ...
            const now = Date.now();
            const justKicked =
                now - this.lastDisconnectTime < 2000 && this.lastKickReason;
            this.lastDisconnectTime = now;
            this.killaura.currentTarget = null;
            if (this.status !== "kicked") this.status = "offline";
            this.logger.info(
                "Bot",
                `[${this.options.username}] 🔌 Disconnect: ${reason || "unknown"}`,
            );
            this.emitUpdate();
        ...
        this.bot.on("playerJoined", (player) => {
            if (this.destroyed) return;
            this.io.to(`bot:${this.id}`).emit("bot:playerJoined", {
                botId: this.id,
        ...
        this.bot.on("playerLeft", (player) => {
            if (this.destroyed) return;
            this.io.to(`bot:${this.id}`).emit("bot:playerLeft", {
                botId: this.id,
                player: { username: player.username, uuid: player.uuid },
            });
        });
        ...
        this.bot.on("entityDead", (entity) => {
            ...
            if (entity.type === "hostile" || entity.kind === "Hostile mobs") {
                this.statsTracker.updateStat(this.id, "mobsKilled");
            }
            if (this.killaura.currentTarget?.id === entity.id) {
                this.killaura.currentTarget = null;
            }
        });
        ...
        this.bot.on("playerCollect", (collector) => {
            if (this.destroyed || !this.bot?.entity) return;
            if (collector?.id === this.bot.entity?.id) {
                this.statsTracker.updateStat(this.id, "itemsCollected");
            }
        });

        this.bot.on("time", () => {
            if (this.destroyed || !this.bot?.time) return;
            this.io.to(`bot:${this.id}`).emit("bot:time", {
                botId: this.id,
        ...
        this.bot.on("rain", () => {
            if (this.destroyed || !this.bot) return;
            this.io.to(`bot:${this.id}`).emit("bot:weather", {
                botId: this.id,
    ...
    startAutoEat(options = {}) {
        if (options.startAt !== undefined)
            this.autoEatState.startAt = options.startAt;
        this.autoEatState.enabled = true;
        this.autoFeatures.eat = true;

        if (!this.intervals.eat) {
            this.intervals.eat = setInterval(() => {
                if (!this.bot || this.status !== "online" || this.destroyed)
                    return;
                if (!this.autoEatState.enabled || this.autoEatState.eating)
                    return;
                if ((this.bot.food || 0) <= this.autoEatState.startAt) {
                    this.tryAutoEat().catch(() => {});
                }
            }, 3000);
        }
        this.logger.info("Bot", `[${this.options.username}] 🍖 Auto-eat BẬT`);
    }
    ...
    stopAutoEat() {
        ...
        if (this.intervals.eat) {
            clearInterval(this.intervals.eat);
            delete this.intervals.eat;
        }
        if (this.autoEatState.eating && this.bot) {
            try {
    ...
    async tryAutoEat() {
        if (
            ...
            return;
        if (Date.now() - this.autoEatState.lastEat < this.autoEatState.cooldown)
            return;
        ...
        try {
            ...
            this.logger.info(
                "Bot",
                `[${this.options.username}] 🍖 Ăn ${food.name} → ${this.bot.food}/20`,
            );
            if (held && held.type !== food.type) {
                await this.sleep(200);
                try {
                    await this.bot.equip(held, "hand");
                } catch (e) {}
            }
        } catch (err) {
            this.logger.warn("Bot", `Auto-eat lỗi: ${err.message}`);
        } finally {
            this.autoEatState.eating = false;
        }
    }
    ...
    getEquipment() {
        ...
        try {
            for (const slot of [
                ...
                const item =
                    this.bot.inventory.slots[
                        this.bot.getEquipmentDestSlot(slot)
                    ];
                eq[slot] = item
                    ? {
                          name: item.name,
    ...
    setHotbarSlot(slot) {
        ...
        if (s < 0 || s > 8) throw new Error("Slot 0-8");
        this.bot.setQuickBarSlot(s);
    }
    ...
    async equipItem(itemName, destination = "hand") {
        ...
        const item = this.bot.inventory
            .items()
            .find((i) => i.name === itemName || i.displayName === itemName);
    ...
    async dropItem(itemName, count = 1) {
        ...
        await this.bot.toss(
            item.type,
            null,
            count === "all" ? item.count : parseInt(count),
        );
    }
    ...
    async digBlock(x, y, z) {
        ...
        if (!this.bot.canDigBlock(block))
            throw new Error(`Không đào được ${block.name}`);
    ...
    handleChatCommand(username, message) {
        ...
        const parts = message.slice(1).toLowerCase().split(" ");
        const cmd = parts[0];
        try {
            switch (cmd) {
                case "help":
                    this.bot.chat(
                        "!pos !health !eat !ka !stop !inv !target !range !speed !priority !fov",
                    );
                    break;
                case "pos": {
                ...
                case "health":
                    this.bot.chat(
                        `❤️ ${Math.round(this.bot.health || 0)}/20 🍖 ${this.bot.food || 0}/20`,
                    );
                    break;
                case "eat":
                    this.tryAutoEat().catch(() => {});
                ...
                case "aura":
                    if (this.killaura.enabled) {
                        this.stopKillaura();
                        this.bot.chat("⚔️ KillAura TẮT");
                    } else {
                        this.startKillaura();
                        this.bot.chat(
                            `⚔️ KillAura BẬT (${this.killaura.cooldownMs}ms | ${this.killaura.priority})`,
                        );
                    }
                    break;
                case "range":
                    if (parts[1]) {
                        this.killaura.range = Math.min(
                            parseFloat(parts[1]) || 4.5,
                            6,
                        );
                        this.bot.chat(`Range: ${this.killaura.range}`);
                    } else {
                        this.bot.chat(`Range: ${this.killaura.range}`);
                    }
                    break;
                case "speed":
                    if (parts[1]) {
                        this.killaura.speed = Math.min(
                            parseFloat(parts[1]) || 0,
                            12,
                        );
                        this.updateCooldown();
                        this.bot.chat(
                            `Speed: ${this.killaura.speed || "auto"} (${this.killaura.cooldownMs}ms)`,
                        );
                    } else {
                        this.bot.chat(
                            `Speed: ${this.killaura.speed || "auto"}`,
                        );
                    }
                    break;
                case "priority":
                    if (
                        parts[1] &&
                        ["angle", "distance", "health"].includes(parts[1])
                    ) {
                        this.killaura.priority = parts[1];
                        this.bot.chat(`Priority: ${this.killaura.priority}`);
                    } else {
                        this.bot.chat(
                            `Priority: ${this.killaura.priority} (angle/distance/health)`,
                        );
                    }
                    break;
                case "fov":
                    if (parts[1]) {
                        this.killaura.fov = Math.min(
                            parseFloat(parts[1]) || 360,
                            360,
                        );
                        this.bot.chat(`FOV: ${this.killaura.fov}°`);
                    } else {
                        this.bot.chat(`FOV: ${this.killaura.fov}°`);
                    }
                    break;
                case "target":
                    this.bot.chat(
                        this.killaura.currentTarget
                            ? `🎯 ${this.killaura.currentTarget.name} (${this.killaura.currentTarget.distance}m) HP:${this.killaura.currentTarget.health || "?"}`
                            : "Không có target",
                    );
                    break;
                case "stop":
                    this.stopAllAuto();
                    this.bot.chat("Dừng tất cả!");
                    break;
                case "inv": {
                    const items = this.getInventory().slice(0, 5);
                    this.bot.chat(
                        `Túi: ${items.map((i) => `${i.name}x${i.count}`).join(", ") || "trống"}`,
                    );
                    break;
                }
            }
    ...
    parseKickReason(reason) {
        ...
        const known = {
            ...
            "multiplayer.disconnect.server_full": "🚫 Server đầy",
            "multiplayer.disconnect.banned": "🔨 Bị ban",
            "multiplayer.disconnect.not_whitelisted": "📋 Không whitelist",
            "You must wait": "⏳ Phải đợi",
            "Connection throttled": "⏳ Quá nhanh",
            "You are already connected": "🔁 Đã kết nối",
        };
        ...
        try {
            ...
            if (p?.value?.translate?.value)
                return (
                    known[p.value.translate.value] || p.value.translate.value
                );
    ...
    handleKick(reason) {
        ...
        if (wm) {
            this.scheduleReconnect((parseInt(wm[1]) + 5) * 1000);
            return;
        }
        if (reason.includes("throttled")) {
            this.scheduleReconnect(35000);
            return;
        }
        if (
            reason.includes("invalid_player_movement") ||
            reason.includes("flying")
        ) {
            this.scheduleReconnect(20000);
            return;
        }
        if (reason.includes("banned") || reason.includes("not_whitelisted")) {
            this.autoReconnect = false;
            return;
        }
        if (reason.includes("server_full")) {
    ...
    scheduleReconnect(delay) {
        ...
        const d = Math.min(
            delay * Math.pow(1.3, Math.min(this.reconnectAttempts, 8)),
            120000,
        );
        ...
        this.logger.info(
            "Bot",
            `[${this.options.username}] 🔄 Reconnect sau ${Math.round(d / 1000)}s (${this.reconnectAttempts}/${this.maxReconnectAttempts})`,
        );
    ...
    destroy() {
        ...
        this.disconnect();
        try {
            this.statsTracker.removeBot(this.id);
        } catch (e) {}
    }
    ...
    stopAllAuto() {
        this.stopAutoEat();
        this.stopKillaura();
        for (const v of Object.values(this.intervals))
            try {
                clearInterval(v);
            } catch (e) {}
        this.intervals = {};
        for (const v of Object.values(this.timers))
            try {
                clearTimeout(v);
            } catch (e) {}
        this.timers = {};
    ...
    getInfo() {
        const info = {
            ...
            killaura: {
                ...
                cooldownMs: this.killaura.cooldownMs,
                totalClicks: this.killaura.totalClicks,
                currentTarget: this.killaura.currentTarget,
                containerOpen: this.killaura.containerOpen,
                targetTypes: this.killaura.targetTypes,
    ...
    emitUpdate() {
        try {
            this.io.emit("bot:update", this.getInfo());
        } catch (e) {}
    }
}
...
class BotManager {
    ...
    createBot(options) {
        ...
        bot.connect();
        this.logger.info("BotManager", `✅ Tạo bot: ${options.username}`);
        return botId;
    }
    ...
    async executeAction(botId, action, params = {}) {
        ...
        switch (action) {
            case "chat":
                bot.sendChat(params.message);
                break;
            case "look":
                await bot.lookAt(params.x, params.y, params.z);
                break;
            case "lookAtEntity":
                await bot.lookAtEntity(params.entityId);
                break;
            case "setLook":
                await bot.setLook(params.yaw, params.pitch);
                break;
            case "attack":
                if (params.entityId) await bot.attackEntity(params.entityId);
                else await bot.attackNearest(params.type || "hostile");
                break;
            case "killaura":
            case "autoClick":
            case "autoFight":
                params.enabled !== false
                    ? bot.startKillaura(params)
                    : bot.stopKillaura();
                break;
            case "dig":
            case "mine":
                await bot.digBlock(params.x, params.y, params.z);
                break;
            case "place":
            ...
            case "equip":
                await bot.equipItem(
                    params.itemName || params.item,
                    params.destination || "hand",
                );
                break;
            case "unequip":
                await bot.unequipItem(params.destination);
                break;
            case "drop":
                params.all
                    ? await bot.dropAllItems()
                    : await bot.dropItem(
                          params.itemName || params.item,
                          params.count,
                      );
            ...
            case "stopUseItem":
                ...
                break;
            case "eat":
                await bot.eatFood(params.itemName || params.item);
                break;
            case "hotbar":
                bot.setHotbarSlot(params.slot);
                break;
            case "autoEat":
                params.enabled !== false
                    ? bot.startAutoEat(params)
                    : bot.stopAutoEat();
                break;
            case "stop":
                bot.stopAllAuto();
